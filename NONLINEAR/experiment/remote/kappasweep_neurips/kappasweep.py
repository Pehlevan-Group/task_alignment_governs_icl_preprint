...
trainobject = finitetasksampler(d, l, n, k, rho, Ctr, 2)
testobject_1 = fulltasksampler(d, l, n, rho, Ctr, 2)
testobject_2 = fulltasksampler(d, l, n, rho, np.diag(spikevalue(d, 0.5, 5)), 2)
...
numsamples = 500
testobject = fulltasksampler(d, l, n, rho, Ctr,2)
tracker = []
...
for test_power in test_powers:
    ...
    testobject = fulltasksampler(d, l, n, rho, Ctest,2)
    tracker = []
    for _ in range(numsamples):
        xs, labels = next(testobject); # generates data
        logits = state.apply_fn({'params': state.params}, xs); # runs xs through transformer and makes predictions
