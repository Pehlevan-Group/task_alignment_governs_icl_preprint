...
from reduced_model_codebase.theory import complexity_class_covariance, ICL_error
...
Ctr = np.diag(complexity_class_covariance(d, int(0.5*d), True)) 
...
Ctest5 = np.diag(spikevalue(d,0,1))

plt.axhline(rho,linestyle=':',linewidth=2, color='grey')
plt.axvline(0.5,linestyle=':',linewidth=2,color='grey')
...
plt.scatter(kappas, [item[1][4] for item in data], color='#FAFAFA', edgecolors=color_cycle[2],s=200,linewidth=3,zorder=10)
...
plt.scatter(kappas, [item[1][0] for item in data], color='#FAFAFA', edgecolors='red',s=200,linewidth=3,zorder=10)
...
plt.plot(kappas, [ICL_error(Ctr, Ctest3_antialigned_power, tau, alpha, kappa, rho, 100) for kappa in kappas], color=color_cycle[5], label = f'Test on aligned powerlaw')
plt.scatter(kappas, [item[1][2] for item in data], color='#FAFAFA', edgecolors=color_cycle[5],s=200,linewidth=3,zorder=10)
...
plt.scatter(kappas, [item[1][1] for item in data], color='#FAFAFA', edgecolors=color_cycle[4],s=200,linewidth=3,zorder=10)
...
plt.plot(kappas, [ICL_error(Ctr, Ctest4_aligned_power, tau, alpha, kappa, rho, 100) for kappa in kappas], color=color_cycle[3], label = f'Test on unaligned powerlaw')
plt.scatter(kappas, [item[1][3] for item in data], color='#FAFAFA', edgecolors=color_cycle[3],s=200,linewidth=3,zorder=10)
