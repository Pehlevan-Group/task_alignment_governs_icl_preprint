...
class LinearSelfAttentionBlock(nn.Module):
    ...
    def __call__(self, inputs, decoder_mask=None, idxs=None):
        ...
        K = self.param("K", lambda key, shape: nn.initializers.lecun_normal()(key, shape), (dp1, dp1))
        Q = self.param("Q", lambda key, shape: nn.initializers.lecun_normal()(key, shape), (dp1, dp1))
        # V = self.param("V", lambda key, shape: nn.initializers.lecun_normal()(key, shape), (dp1, dp1))
        V = self.param(
            "V",
            lambda key, shape: jnp.eye(shape[0], shape[1], dtype=jnp.float32),
            (dp1, dp1),
        )
        ...
        # # logits: (b, l, l)
        # logits = jnp.einsum("bld,bdm->blm", jnp.swapaxes(QZ, 1, 2), KZ)  # Q^T K
        # logits = logits / jnp.sqrt(dp1-1)
        # # stable softmax over keys axis
        # P = jax.nn.softmax(logits, axis=-1)  # (b, l, l)
        # # apply weights to values
        # out = jnp.einsum("bdm,blm->bdl", VZ, P)  # (b, d, l)
        # A = Z + out
